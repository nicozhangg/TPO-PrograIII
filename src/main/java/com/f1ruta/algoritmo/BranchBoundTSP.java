...
public class BranchBoundTSP {
    ...
    /** Heurística mejorada para casos grandes (más de 20 nodos) */
    private static Resultado calcularRutaHeuristica(List<Circuito> cs, String origenNombre) {
        int n = cs.size();
        double[][] distancias = distancias(cs);
        ...
        // Probar múltiples inicios y elegir el mejor
        List<Integer> mejorRuta = null;
        double mejorCosto = Double.POSITIVE_INFINITY;
        
        // Probar desde el origen y algunos vecinos cercanos
        int[] inicios = {origen};
        if (n > 15) {
            // Solo el origen para casos grandes
        } else {
            // Probar varios orígenes para casos medianos
            inicios = new int[Math.min(5, n)];
            for (int i = 0; i < inicios.length; i++) {
                inicios[i] = (origen + i) % n;
            }
        }
        
        for (int inicio : inicios) {
            List<Integer> ruta = vecinoMasCercano(distancias, inicio);
            double costo = calcularCostoRuta(distancias, ruta);
            
            // Aplicar mejoras 2-opt
            ruta = optimizar2Opt(distancias, ruta);
            costo = calcularCostoRuta(distancias, ruta);
    ...
    /** Optimización 2-opt local */
    private static List<Integer> optimizar2Opt(double[][] d, List<Integer> ruta) {
        List<Integer> mejor = new ArrayList<>(ruta);
        boolean mejora = true;
        int n = mejor.size();
        ...
        while (mejora) {
            ...
            for (int i = 1; i < n - 2; i++) {
                for (int j = i + 1; j < n - 1; j++) {
                    int a = mejor.get(i - 1), b = mejor.get(i);
                    int c = mejor.get(j), e = mejor.get(j + 1);
                    double delta = (d[a][c] + d[b][e]) - (d[a][b] + d[c][e]);
                    if (delta < -1e-6) {
                        Collections.reverse(mejor.subList(i, j + 1));
                        mejora = true;
                    }
                }
            }
